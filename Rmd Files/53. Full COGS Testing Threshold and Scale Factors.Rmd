...
## Getting Cogs Counts
```{r}
cogs_cs <- df_COGS2[df_COGS2$cDiagnosis4 == 'CS',]
cogs_counts_cs <- plyr::count(cogs_cs, c('cRace2', 'cHispanicorLatino', 'cGender'))
...
cogs_sz <- df_COGS2[df_COGS2$cDiagnosis4 == 'SZSAFD',]
cogs_counts_sz <- plyr::count(cogs_sz, c('cRace2', 'cHispanicorLatino', 'cGender'))
...
start_time <- Sys.time()
for (i in 1:n_sim) {
  steps_storage[i] <- sampling_algorithm2(cogs_og = cogs_counts_cs, pool_df = pool_cs, scale_factor = 0.3, thresh = 0.025, n_iter = 1000, track_rej = FALSE, suppress_df = TRUE)
...
```{r}
pdf('53. Testing Th and Sf 03.pdf')
...
# 
# plots.png.paths
# file.copy(from=plots.png.paths, to=c('03_1.png', '03_2.png', '03_3.png'))
```

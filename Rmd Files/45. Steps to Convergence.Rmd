...
```{r}
prop_ct_cs <- plyr::count(no_sd[no_sd$cDiagnosis4 == 'CS',], c('cRace2', 'cHispanicorLatino', 'cGender'))
...
start_time <- Sys.time()
for (i in 1:n_sim) {
  steps_storage_cs_acs[i] <- sampling_algorithm(cogs_og = cogs_counts, pool_df = pool_cs, scale_factor = 0.5, thresh = 0.025, n_iter = 1000, suppress_df = TRUE)
}

end_time <- Sys.time()
end_time - start_time
rng <- range(steps_storage_cs_acs)
hist(steps_storage_cs_acs, main = 'Number of Resamples until Convergence', breaks = rng[1]:rng[2], xlab = "Number of Resamples")
hist(steps_storage_cs_acs, freq = FALSE, main = 'Number of Resamples until Convergence')

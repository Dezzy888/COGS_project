...
city_counts <- plyr::count(no_sd[no_sd$cDiagnosis4 == 'SZSAFD', ], 'cLocationCity')
...
# # weighting the frequencies in cityprops by the above proportions
# cityprops_weighted <- vector(mode = 'list', 4)
# for (i in 1:length(cityprops)) {
#   cityprops_weighted[[i]] <- cityprops[[i]][,1:4]
#   cityprops_weighted[[i]]$weighted_freq <- cityprops_weighted[[i]]$freq * city_counts$proportion[i]
# }
# 
# total_pool <- sum(cityprops_weighted[[1]]$weighted_freq,
#     cityprops_weighted[[2]]$weighted_freq,
#      cityprops_weighted[[3]]$weighted_freq,
#       cityprops_weighted[[4]]$weighted_freq)
# 
# for (i in 1:length(cityprops_weighted)) {
#   cityprops_weighted[[i]]$weighted_prop <- cityprops_weighted[[i]]$weighted_freq / total_pool
# }
# 
# # sums to 1, as expected
# # sum(cityprops_weighted[[1]]$weighted_prop,
# #      cityprops_weighted[[2]]$weighted_prop,
# #       cityprops_weighted[[3]]$weighted_prop,
# #        cityprops_weighted[[4]]$weighted_prop)
...
for (i in 1:length(cogs_codes)) {
  acs1865$PERWT_SZ[acs1865$CITY == cogs_codes[i]] <- acs1865$PERWT[acs1865$CITY == cogs_codes[i]] * city_counts$proportion[i]
}
...
## PERWT Demonstration
```{r}
acs1865_sub <- acs1865[, c('CITY','Race2','Hispan2','SEX','PERWT','PERWT_SZ')]
...
all(city_counts$proportion[1] * acs1865_LA$PERWT == acs1865_LA$PERWT_SZ)

# Showing that the original sample size is recovered
sum(acs1865_LA$PERWT_SZ)
sum(acs1865_NY$PERWT_SZ)
city_counts
...
df_PERWT <- data.frame(Race2 = acs1865_LA$Race2,
           ...
           rescaled_PERWT = acs1865_LA$PERWT_SZ)
head(df_PERWT)
```
...
pool_sz <- plyr::count(acs1865, c('Race2', 'Hispan2', 'SEX'), wt_var = 'PERWT_SZ')
...
```{r}
no_sd_cs <- no_sd[no_sd$cDiagnosis4 == 'SZSAFD',]
...
pool_sz2 <- pool_gen('SZSAFD',no_sd,acs1865,group_codes_df)
identical(pool_sz2, pool_sz) # the pool generator function works
```

## Checking proportions
```{r}
prop_ct_sz <- plyr::count(no_sd[no_sd$cDiagnosis4 == 'SZSAFD',], c('cRace2', 'cHispanicorLatino', 'cGender'))
...
for (i in 1:nrow(df_comparison_sz)) {
  test_obj <- binom.test(df_comparison_sz$freq[i], n = sum(prop_ct_cs$freq), p = df_comparison_sz$`pool_sz_trunc$prop`[i], alternative = 'two.sided')
...
start_time <- Sys.time()
for (i in 1:n_sim) {
  steps_storage_sz_acs[i] <- sampling_algorithm(cogs_og = cogs_counts, pool_df = pool_sz, scale_factor = 0.5, thresh = 0.025, n_iter = 1000, suppress_df = TRUE)
}

end_time <- Sys.time()
end_time - start_time
rng<-range(steps_storage_sz_acs) # getting the range of the simulation results
hist(steps_storage_sz_acs, main = 'Number of Resamples until Convergence', breaks = rng[1]:rng[2], xlab = 'Number of Resamples')
hist(steps_storage_sz_acs, freq = FALSE, main = 'Number of Resamples until Convergence')
...
# data.frame(Median = med_step,
#            lower_2.5 = conf_int[1],
#            upper_97.5 = conf_int[2])

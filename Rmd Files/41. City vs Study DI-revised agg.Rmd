---
title: "Census vs Study Diversity Indices"
...
### Aggregate DI
```{r}
#pdf(file='Census_vs_Study_div_index.pdf')
head(df_COGS2)
...
cogs_cities_acs2 <- c(-1,cogs_cities_acs) # -1 is surrogate for aggregate
...
for (i in 1:length(cogs_cities_acs2)) {
  for (d in 1:length(diagnoses)) {
    ...
    if (cogs_cities_acs2[i] != -1) {
      ...
      # Get counts of each combination, but per city
      weighted_cts <- plyr::count(df_sub, c('Race2', 'Hispan2', 'SEX','CITY'), wt_var = 'PERWT')
      
      # Generate within-city totals
      tots <- plyr::count(df_sub, 'CITY', wt_var = 'PERWT')
      
      # Generate proportions of each combination within each city
      weighted_cts$props <- rep(0, nrow(weighted_cts))
      
      for (r in 1:nrow(weighted_cts)) {
        weighted_cts$props[r] <- weighted_cts$freq[r] / tots$freq[tots$CITY == weighted_cts$CITY[r]]
      }
      
       for (N in 1:N_sim) {
         
         # splitting the weighted_cts into 1 df for each city
         df_LA <- weighted_cts[weighted_cts$CITY == 3730,]
         df_NY <- weighted_cts[weighted_cts$CITY == 4610,]
         df_Ph <- weighted_cts[weighted_cts$CITY == 5330,]
         df_Se <- weighted_cts[weighted_cts$CITY == 6430,]
         ...
         # sample sizes
         ss <- city_ns$freq[city_ns$cDiagnosis4 == diagnoses[d]][1:4]
         
        row_samples <- c(sample(1:nrow(df_LA), size = ss[1], p = df_LA$props, replace = TRUE),
                         sample(1:nrow(df_NY), size = ss[2], p = df_NY$props, replace = TRUE),
                         sample(1:nrow(df_Ph), size = ss[3], p = df_Ph$props, replace = TRUE),
                         sample(1:nrow(df_Se), size = ss[4], p = df_Se$props, replace = TRUE))
...
acs_di$Percent_Max_DI <- acs_di$DI / log(24)
sim_res <- data.frame( city = rep(cogs_cities_acs2, rep(2, 5)), sim_means  = apply(sim_mat, 1, mean))
sim_res
city_ns
...
write.csv(di_sig, file = 'div_idx_resam_test.csv', row.names = FALSE)
write.csv(acs_di, file = 'ACS_DI.csv', row.names = FALSE)

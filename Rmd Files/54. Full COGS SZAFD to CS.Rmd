...
* This is just the observed proportions seen in COGS controls 
```{r}
...
* 50% --> 30% of original SZ sample size
* 0.025 threshold
```{r}
n_sim <- 1000
...
start_time <- Sys.time()
for (i in 1:n_sim) {
  steps_storage[i] <- sampling_algorithm2(cogs_counts_sz, pool_cs_complete$prop, 0.3, 0.025, 1000, track_rej = FALSE, suppress_df= TRUE)
}
...
hist(steps_storage, freq = TRUE, xlim = c(0, max(steps_storage)), breaks = 0:rng[2], xlab = 'Number of Resamples', main = 'SZSAFD to CS: Resamples\nuntil Convergence')
title(sub = paste('Scale factor:', '0.3', '\tThreshold:', '0.025'), font.sub = 3, col.sub = 'darkgray')
grid()
...
tbl <- kbl(sum_stats, caption = "Number of Resamples until SZSAFD Convergence to Control Subjects", table.attr = "style='width:50%;'" ) %>%
